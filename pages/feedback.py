...
if (correct_log or coaching_log) and not st.session_state.get("current_session_id"):
    ...
    <style>
      ...
      .sp-label {
        font:600 15px/1 system-ui,-apple-system,BlinkMacSystemFont,Roboto,sans-serif;
...
def _render_vocab(s):
    ...
    sid = s["id"]
    st.markdown(
        "<div style='font:700 10px system-ui;letter-spacing:2px;color:rgba(17,24,39,.4);"
    ...
    for i, item in enumerate(vocab):
        ...
        with cols[i % 2]:
            st.markdown(
                ...
                f"    <span style='font:700 15px system-ui;color:#111827'>{word}</span>"
                f"    <span style='font:400 11px system-ui;color:rgba(17,24,39,.5)'>{trans}</span>"
                f"  </div>"
                f"  <div style='font:400 12px/1.5 system-ui;color:rgba(17,24,39,.6);"
...
def render_session(s):
    ...
    correct_chip = (
        ...
        f"border-radius:20px;padding:3px 12px;font:600 11px system-ui;color:{v_color};"
    ...
    st.markdown(
        ...
        f"  <div style='font:500 12px system-ui;color:{v_color};letter-spacing:.5px;"
        f"text-transform:uppercase;margin-bottom:10px'>Would a native speaker understand you?</div>"
        f"  <div style='font:800 26px/1.1 system-ui,-apple-system,BlinkMacSystemFont,Roboto,sans-serif;color:{v_color};"
        f"letter-spacing:-.3px;margin-bottom:8px'>{verdict}</div>"
        f"  <div style='font:400 13px/1.6 system-ui;color:rgba(17,24,39,.6);"
        ...
        f"    <span style='font:400 12px system-ui;color:rgba(17,24,39,.35)'>"
    ...
    if errors:
        ...
        for i, err in enumerate(errors):
            ...
            st.markdown(
                ...
                f"  <span style='font:700 11px system-ui;color:rgba(13,148,136,.5)'>{n:02d}</span>"
                f"  <span style='font:500 14px/1.4 system-ui;color:#111827'>{q}</span>"
                f"</div>"
                # Two columns: You said / Correction
                f"<div style='display:grid;grid-template-columns:1fr 1.5fr;gap:12px'>"
                # You said
                f"  <div>"
                f"    <div style='font:600 9px system-ui;color:rgba(17,24,39,.4);letter-spacing:.8px;"
                f"text-transform:uppercase;margin-bottom:6px'>You said</div>"
                f"    <div style='font:400 13px system-ui;color:rgba(17,24,39,.5);"
                ...
                f"    <div style='font:400 13px/1.5 system-ui;color:#0f3d39;"
        ...
        st.markdown(
            ...
            "<div>"
            "  <div style='font:600 15px system-ui;color:#059669;margin-bottom:3px'>Clean session</div>"
            "  <div style='font:400 12px system-ui;color:rgba(5,150,105,.55)'>No mistakes this time — great work!</div>"
            "</div></div>",
            unsafe_allow_html=True
        )
    ...
    if dynamic_tip or static_tip:
        ...
        st.markdown(
            ...
            f"  <span style='font:600 10px system-ui;color:rgba(17,24,39,.45);letter-spacing:1.5px;"
            f"text-transform:uppercase'>Tip for {s_bg} speakers</span>"
            f"</div>"
            f"<div style='font:400 12px/1.7 system-ui;color:rgba(17,24,39,.6)'>{tip_text}</div>"
            f"</div>",
            unsafe_allow_html=True
        )
            ...
            for entry in conv:
                ...
                if entry["who"] == "character":
                    parts.append(
                        ...
                        f"<span style='font:600 9px system-ui;color:rgba(17,24,39,.4);text-transform:uppercase;"
                        f"letter-spacing:.5px;display:block;margin-bottom:4px'>{char_lbl}</span>"
                        f"<span style='font:400 13px/1.4 system-ui;color:#111827'><em>{txt}</em></span></div>"
                    )
                elif entry["who"] == "tutor":
                    parts.append(
                        f"<div style='background:rgba(245,158,11,.07);border-radius:4px 14px 14px 14px;"
                        f"padding:10px 14px;margin:6px 0;max-width:78%'>"
                        f"<span style='font:600 9px system-ui;color:#d97706;text-transform:uppercase;"
                        f"letter-spacing:.5px;display:block;margin-bottom:4px'>💡 {tutor_disp}</span>"
                        f"<span style='font:400 13px/1.4 system-ui;color:#78350f'>{txt}</span></div>"
                    )
                else:
                    parts.append(
                        ...
                        f"<span style='font:400 13px/1.4 system-ui;color:#0f3d39'>{txt}</span></div>"
                    )
            st.markdown(
...
<div style='margin-bottom:32px;padding:28px 32px;
  ...
  <div style='font:800 28px/1 system-ui,-apple-system,BlinkMacSystemFont,Roboto,sans-serif;color:#111827;letter-spacing:-.5px;
...
with tab_latest:
    if history:
        ...
        st.markdown(
            ...
            "<div style='font:700 16px system-ui,-apple-system,BlinkMacSystemFont,Roboto,sans-serif;color:#111827;margin-bottom:6px'>"
            "Keep the momentum going</div>"
            f"<div style='font:400 13px system-ui;color:rgba(17,24,39,.55)'>"
